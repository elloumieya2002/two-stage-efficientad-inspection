...
```bash
python scripts/retrain_efficientad.py \
    --dataset_path ./cropped_dataset \
    --ckpt_dir     ./ckpt_cropped \
    --teacher_path /path/to/pretrained/best_teacher.pth \
    --category     chewinggum \
    --model_size   S \
    --iterations   70000 \
    --device       cuda
...
```bash
# With YOLO detection (original VisA full-resolution images):
