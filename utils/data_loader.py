            ...
            for row in csvreader:
                ...
                if self.phase == split and self.category == category :
                    ...
                    if label == "normal":
                        ...
                        index = 1
                        types = "bad"
                        # mask_path is "" when no GT mask was cropped (simple pipeline).
                        # os.path.join(root, "") returns root dir → IsADirectoryError.
                        # Treat missing mask as gt=0 (no pixel-level ground truth).
                        gt_src_path = os.path.join(self.root, mask_path) if mask_path else 0
